...
Grafo leitura(const char *nomeDoArquivo){
	...
	int n, m;
	/*pega o numero de vertices e arestas da primeira linha do arquivo*/
	token = strtok(buffer, " ");
	n = atoi(token);
	token = strtok( NULL, " " );
	m = atoi(token);
	/*fgets pra pular o primeiro -1*/
	fgets(buffer, 1024, arq);
	...
	/*while que le o resto do arquivo*/
	while(!feof(arq)){
		fgets(buffer, 1024, arq);
		/*if pra mudar a flag de leitura de vertices pra leitura de arestas*/
		if ( strcmp("-1\n", buffer) == 0){
			flagAresta = 1;
			continue;
		}
		/*quebra a linha e pega o peso dos vertices*/
		if( flagAresta == 0 ){
			token = strtok(buffer, " ");
			token = strtok( NULL, " " );
			token = strtok( NULL, " " );
		...
		/*quebra a linha e pega o peso das arestas*/
		if( flagAresta == 1 ){
			if( strlen(buffer) < 5 ){
				break;
			}
